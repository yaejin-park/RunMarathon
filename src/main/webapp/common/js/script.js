$(function(){
	...
	$(".accor-title").click(function(){
		if(!$(this).next().is(":visible")){
			$(this).parent().parent().siblings().find(".accor-content").slideUp();
			$(this).next().slideDown();
			$(this).find('.glyphicon-menu-down').css('color','#659EFF').removeClass("glyphicon-menu-down").addClass("glyphicon-menu-up");
			$(this).parent().parent().siblings().find('.glyphicon-menu-up').css('color','gray').removeClass("glyphicon-menu-up").addClass("glyphicon-menu-down");
		} else{
			$(this).next().slideUp();
